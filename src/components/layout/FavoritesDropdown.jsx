const FavoritesDropdown = ({ favorites, getSelectedLocation, closeDropdown, removeFavorite }) => {
  const FavoritesDropdownStyles =
    'custom-scrollbar bg-neutral-800 absolute z-20 top-16 mobile:min-w-[320px] w-full max-h-[184px] rounded-12 p-100 border-1 border-neutral-700 overflow-y-scroll'

  function handleItemClick(location) {
    getSelectedLocation(location)
    closeDropdown('favorites-dropdowns')
  }

  function handleDeleteButton(currentFavorite, event) {
    ...
    <ul id="favorites-dropdown" className={FavoritesDropdownStyles}>
      {favorites.length === 0 ? (
        ...
        favorites.map((fav) => {
          return (
            <li
              className="text-neutral-0 text-preset-7 rounded-8 flex cursor-pointer items-center justify-between border-1 border-transparent px-100 py-125 transition-colors hover:border-neutral-600 hover:bg-neutral-700"
              key={fav.latitude}
            ...
            >
              ...
              <button
                onClick={(event) => handleDeleteButton(fav, event)}
                className="rounded-8 group cursor-pointer p-50 transition-colors hover:bg-neutral-900/30"
              >
                <svg width="24px" height="24px" viewBox="0 -4 24 28" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path
                    ...
                    className="origin-bottom-right fill-neutral-200 transition-transform group-hover:translate-y-[-4px] group-hover:rotate-[-8deg]"
                  />
                  <path
